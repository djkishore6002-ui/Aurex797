      ...
      <div className="w-full max-w-2xl">
        <div className={`card overflow-hidden ${valid ? '' : 'border-red-200'}`}>
          ...
          <div className="p-8">
            ...
            <div className={`rounded-2xl border-4 p-8 text-center ${valid ? 'border-marigold-400 bg-gradient-to-br from-white to-marigold-50' : 'border-red-300 bg-red-50'}`}>
              <p className="tamil text-sm tracking-[0.3em] text-ink-400">சோலை · SOLAI</p>
              <p className="mt-4 text-xs font-semibold uppercase tracking-[0.25em] text-ink-400">{cert.template_title ?? cert.title_text}</p>
              <p className="tamil mt-6 text-2xl font-bold text-ink-900">This certifies that</p>
              <p className="mt-3 text-3xl font-extrabold tracking-tight text-brand-800">{cert.participant_name}</p>
              <p className="mt-4 text-sm text-ink-500">{cert.template_body ?? 'is hereby recognized for successfully completing'}</p>
              <p className="mt-2 text-xl font-bold text-ink-900">{cert.title_text}</p>
              <div className="mt-6 flex flex-wrap items-center justify-center gap-x-8 gap-y-2 text-sm text-ink-600">
              ...
              <div className="mt-8 flex items-center justify-center gap-8 border-t border-ink-100 pt-5 text-xs text-ink-400">
                <span>Issued {fmtDate(cert.issued_at)}</span>
                <span>Certificate ID: <b className="text-ink-700">{cert.certificate_id}</b></span>
                <span>Verified {fmtDate(new Date().toISOString())} · {cert.verificationCount}× checked</span>
              </div>
              {cert.template_signature && <p className="tamil mt-5 font-semibold text-ink-700">{cert.template_signature}</p>}
            </div>
